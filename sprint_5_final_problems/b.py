...
#
# ПРИНЦИП РАБОТЫ
# Находим вершину, которую необходимо удалить и рассматриваем три случая:
#
# 1. У вершины нет детей.
# 2. У вершины только один ребёнок.
# 3. У вершины два ребёнка.
#
# В первом случае просто затераем ссылку на эту вершину.
# Во втором случае трансплантируем ребёнка к родителю вершины, которую удаляем.
# В третьем случае найдём предшественника вершины, которую удаляем. При этом отдельно обработаем случай,
# когда у предшественника есть левый ребёнок, сохраняя на него ссылку, если таковой имеется. После этого удалим
# необходимую вершину и на её место установим предшественника.
#
# Отдельное внимание уделяем тому, что будет являтся корнем результируещего дерева.
# Для удобства мы вводим дополнительный рутины для поиска максимума и поиска ноды по её значению.
#
# --- ВРЕМЕННАЯ СЛОЖНОСТЬ
# O(h), где h - высота дерева. Действительно, само удаление - константная операция, а поиск ноды и
# поиск максимума занимают время, пропорциональное высоте дерева, т.е. h.
#
# --- ПРОСТРАНСТВЕННАЯ СЛОЖНОСТЬ
# O(1) - так как не требуется дополнительного места.
